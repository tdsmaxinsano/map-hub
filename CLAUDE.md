...
| `09_clinician_vdr_summary.sql` | Adds `list_clinician_vdr_summary()` SECURITY DEFINER RPC that aggregates `vdr_clinician_metrics` per clinician — returns `run_count`, `total_visits`, `avg_visits_per_run`, `avg_sync_days` (visit-weighted), `total_charges`, `latest_run_at`. Surfaces in Roster Review (two toggleable columns) + clinician profile in the map (two chips in the header quick-facts row). Aggregates are SECURITY DEFINER so editors see the numbers even though per-run rows remain admin-only. Idempotent. |
