-- Clinician "Requested areas" — where a therapist ASKS to work
-- =====================================================================
-- The positive mirror of the Won't-serve list (migration 56). Therapists
-- request areas ("Farrah: Mt Prospect + Des Plaines") — today that lives
-- in a TB memo field where no staffing tool sees it. This records it as
-- ZIPs so it's recognized wherever staffing happens:
--
--   • Suggest engines show a green "✋ Requested area" chip when the
--     referral's ZIP is on the candidate's request list — a positive
--     signal for coordinators (display-only; ranking unchanged in v1).
--   • The Territory Planner lists requests with per-ZIP status (already
--     declared in TB ✓ / not declared yet → stage it in the move plan).
--
--   clinician_zip_requests — one row per (clinician, requested ZIP),
--   note + who/when. Managed from the Territory Planner's ✋ card
--   (city names resolve to ZIPs like the Won't-serve collector).
--
-- suggest_clinicians_for_referral is REBUILT to add zip_requested
-- BOOLEAN (return-shape change → DROP first; body = migration 56's).
--
-- RLS: SELECT authenticated; writes admin+editor. Idempotent.
-- Run after 56_clinician_zip_exclusions.sql.

BEGIN;

-- ── 1. Table ─────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.clinician_zip_requests (
...
COMMENT ON TABLE public.clinician_zip_requests IS
  'Areas a clinician has ASKED to work ("requested areas"). Suggest engines show a positive ✋ chip for referrals in these ZIPs; the Territory Planner tracks declared-in-TB status per request.';

ALTER TABLE public.clinician_zip_requests ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "zip_req_select_auth"        ON public.clinician_zip_requests;
DROP POLICY IF EXISTS "zip_req_write_editor_admin" ON public.clinician_zip_requests;

CREATE POLICY "zip_req_select_auth" ON public.clinician_zip_requests
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "zip_req_write_editor_admin" ON public.clinician_zip_requests
...
-- ── 2. Rebuild the suggest RPC with zip_requested ────────────────────
...
SELECT
  ...
  (zq.clinician_id IS NOT NULL)                                   AS zip_requested
FROM ids
...
-- "Requested area" — the candidate ASKED to work this ZIP.
LEFT JOIN public.clinician_zip_requests zq
  ON zq.clinician_id = cv.id AND pr.zip IS NOT NULL AND zq.zip_code = pr.zip
